...
pub trait Sequence<V>: Sized {
    ...
    fn build(precursor: Self::Builder) -> Result<Self, TableIntoError>;
}

impl<V> Sequence<V> for Vec<V> {
    type Builder = MaybeVec<V>;

    fn build(maybe_vec: MaybeVec<V>) -> Result<Self, TableIntoError> {
...
impl<const N: usize, V> Sequence<V> for [Option<V>; N] {
    type Builder = Self;
    fn build(this: Self) -> Result<Self, TableIntoError> {
        Ok(this)
    }
}
...
impl<const L: usize, V> Sequence<V> for LimitedVec<L, V> {
    ...
    fn build(maybe_vec: Self) -> Result<Self, TableIntoError> {
        Ok(maybe_vec)
    }
}
...
impl<V> Table<V> {
        ...
        EF: FnMut(TableIntoError) -> E,
    {
        ...
        let cont = S::build(maybe_cont).map_err(&mut err_map)?;
        Ok(cont)
    }
...
impl<V> TableLoadBuilder<V> {
    ...
    pub(crate) fn build<E: LoadError>(self) -> Result<Table<V>, E> {
        let Self { array, assoc } = self;
        let assoc = assoc.build::<E>()?;
